...
import com.cs.common.pool.ConnectionPool;
...
public class BatchScreen extends JApplet {
	private static class MainPanel extends JPanel {
		...
		private class AcquireImageAction extends AbstractAction implements
				Runnable {
			AcquireImageAction() {
				super("acquire image");
			}
			...
			public synchronized void run() {
				try {
					...
					JOptionPane
							.showMessageDialog(
									MainPanel.this,
									"Previous version of Morena is installed in "
											+ System.getProperty("java.home")
											+ ".\nYou have to remove it first.\nClick OK to terminate browser.",
		...
		private class SaveImageAction extends AbstractAction implements
				Runnable {
			...
			public synchronized void run() {
				try {
					...
					// Java 1.6 introduced a comfortable
					// ImageIO.getWriterFileSuffixes() method.
		...
		private class UploadImageAction extends AbstractAction implements
				Runnable {
			UploadImageAction() {
			...
			public synchronized void run() {
				try {
					...
					while ((c = in.read()) != -1)
						System.err.write(c);
					in.close();
					URL imageURL = new URL(documentBase, connection
							.getHeaderField("file-name"));
					status.setText("Done - image is uploaded to " + imageURL
		...
		private class ImagePanel extends JPanel {
			...
			public void paint(Graphics g) {
				...
				if (horizontalRatio > verticalRatio)
					g.drawImage(image, (int) (panelWidth - imageWidth
							* verticalRatio) / 2 + 3, 3,
							(int) (imageWidth * verticalRatio),
							(int) (imageHeight * verticalRatio), this);
				else
					g.drawImage(image, 3, 3,
							(int) (imageWidth * horizontalRatio),
							(int) (imageHeight * horizontalRatio), this);
			}
		...
		MainPanel(Container container, URL documentBase) {
			...
			hasServer = documentBase != null
					&& documentBase.getProtocol().indexOf("http") != -1;
